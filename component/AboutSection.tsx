...
function AnimatedCounter({ target, suffix = "" }: { target: number; suffix?: string }) {
  ...
  return (
    <div ref={ref}>
      <span className="font-syne text-5xl md:text-6xl font-extrabold text-brand-accent">
        {count}{suffix}
      </span>
    </div>
  );
...
export default function AboutSection() {
  ...
  return (
    <section
      id="ethics"
      className="py-40 bg-surface relative overflow-hidden border-y border-white/5"
    >
      {/* BG texture image */}
      <div className="absolute top-0 right-0 w-1/2 h-full opacity-[0.07] grayscale pointer-events-none">
        <img
      ...
      <div
        ref={ref}
        className={`container mx-auto px-6 md:px-20 relative z-10 transition-all duration-1000 ${
      ...
      >
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-24 items-center">
          ...
          <div className="space-y-10">
            <div>
              <p className="font-syne text-[10px] md:text-xs tracking-[0.4em] text-brand-accent uppercase font-bold mb-6">
                The Eye Behind the Lens
              </p>
              <h2 className="font-syne text-4xl md:text-6xl font-bold leading-tight text-white">
            ...
            <blockquote className="border-l-2 border-brand-accent pl-6 py-2">
              <p className="font-syne text-lg italic text-white/70 leading-relaxed">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              {["National Geographic", "BBC Wildlife", "Sony World Photography"].map((award) => (
                <span
                  key={award}
                  className="border border-white/10 px-4 py-2 font-syne text-[9px] tracking-widest uppercase text-white/40 hover:border-brand-accent hover:text-brand-accent transition-colors duration-300 cursor-default"
                >
                  {award}
                </span>
              ))}
          ...
          <div className="grid grid-cols-2 gap-10">
            {[
              ...
              <div
                key={label}
                className="p-6 border border-white/5 hover:border-brand-accent/30 transition-colors duration-500 group"
              >
